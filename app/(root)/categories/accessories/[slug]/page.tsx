...
import { IAccessory } from "@/utils/models"
...
export default async function Accessory({ params, }: { params: Promise<{ slug: string }> }) {
    ...
    const { accessory }: { accessory: IAccessory } = await response.json()

    return (
        <>
            <section>
                <div className="section-container pt-0 pb-25 border-b border-zinc-900">
                    <Link href="/categories/accessories"><p className="flex items-center gap-3 hover:text-zinc-100 transition-colors"><ChevronLeft width={14} height={14} /> Back to accessories&apos;s collection</p></Link>
                    <div className="pt-12 flex gap-20">
                        ...
                        <div>
                            <div className="pt-6 pb-10 flex flex-col gap-6 border-b border-zinc-900">
                                ...
                                <h2 className="font-normal text-left">{accessory.title}</h2>
                                <h3>${accessory.price}</h3>
                                <p className="text-zinc-400">{accessory.description}</p>
                            ...
                            <div className="py-6 flex flex-col gap-5">
                                ...
                                <div className="flex justify-between">
                                    <p>Material</p>
                                    <p className="text-zinc-100">{accessory.material}</p>
                                </div>
                                <div className="flex justify-between">
                                    <p>Fit</p>
                                    <p className="text-zinc-100">{accessory.fit}</p>
                                </div>
                                <div className="flex justify-between">
                                    <p>Care</p>
                                    <p className="text-zinc-100">{accessory.care}</p>
                                </div>
                                <div className="flex justify-between">
                                    <p>Origin</p>
                                    <p className="text-zinc-100">{accessory.origin}</p>
                                </div>
                            </div>
                        </div>
                    </div>
